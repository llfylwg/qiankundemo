<template>
    <div class="goods-info-wrap">
      <p class="detail-title">商品快照</p>
      <p class="quickPic-tip">
        <i class="iconfont icon-dengpao"></i>
        页面内容为订单快照，包含订单创建时商品和下单信息，发生交易争议时，该页面作为判定依据。</p>
      ...
      <el-card class="box-card" v-if="detailData">
        <div class="detail-box">
          <section class="item-box">
            ...
            <span v-for="(item, key) in priceInfo" :key="key" class="text item">
              {{item.value + detailData[item.key] + item.unit}}
            </span>
          </section>
        </div>
      </el-card>
...
<script>
  ...
  export default {
    ...
    created () {
      this.$http.get('wholesale/order/prodSnapshot', {subOrderId: this.$route.query['subOrderId']}).then(res => {
